...
def sign_in_page():
    ...
    # تحويل الحاوية إلى Form ليدعم الضغط على Enter
    # قمنا بإلغاء الإطار الافتراضي للـ Form عبر clear_on_submit واعتمدنا على الحاوية الأنيقة الخاصة بك
    with st.container(border=True):
        st.markdown('<p class="form-label">Connectez-vous pour continuer</p>', unsafe_allow_html=True)
        
        # إنشاء الفورم (مع إخفاء زر التقديم الافتراضي البشع عبر تجزئته بالأسفل)
        with st.form(key="login_form", clear_on_submit=False):
            ...
            with col_center:
                # 👈 هنا السحر: استبدلنا st.button بـ st.form_submit_button
                # هذا الزر الآن مربوط بريفريش الـ Enter تلقائياً
